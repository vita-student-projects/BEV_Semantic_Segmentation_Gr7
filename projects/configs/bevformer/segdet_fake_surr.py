...
_task_={'seg': True, 'det': True}

black = 0
white = 255
gray = 128

_cover_ = {'cover':True, 'cover_value': gray, 'cover_w':-1}
...
model = dict(
    ...
    only_front=_only_front_,
    gd_size = _gd_size_,

    #------------Backbone-----------#
    img_backbone=dict(
        type='ResNet', # mmdet.models.backbones.resnet.py
        depth=101,
        num_stages=4,
        out_indices=(3,),
        frozen_stages=1,
        norm_cfg=dict(type='BN2d', requires_grad=False),
        norm_eval=True, # freeze the batchnorm layer
        style='caffe',
    ...
    pts_bbox_head=dict(
        ...
        #----------segmentation loss----------#
        loss_seg=dict(
            type='CrossEntropyLoss', # mmdet.models.losses.cross_entropy_loss.py
            use_sigmoid=False,
            loss_weight=3.0,
        ...
        #----------detection loadd---------#
        loss_cls=dict(
            type='FocalLoss', # mmdet.models.focal_loss.py
            use_sigmoid=True,
            gamma=2.0,
            alpha=0.25,
            loss_weight=2.0),
        loss_bbox=dict(type='L1Loss', loss_weight=0.25), # mmdet.models.smooth_l1_loss.py
        loss_iou=dict(type='GIoULoss', loss_weight=0.0)), # mmdet.models.iou_loss.py, fake loss
...
train_pipeline = [
    dict(type='CustomLoadMultiViewImageFromFiles', to_float32=True, cover=_cover_),
    dict(type='PhotoMetricDistortionMultiViewImage'),
    dict(type='LoadAnnotations3D', with_bbox_3d=True, with_label_3d=True, with_attr_label=False),
    # convert map labels
    dict(type='RasterizeMapVectors', data_conf=data_conf),
