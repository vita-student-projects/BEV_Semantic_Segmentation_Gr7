...
from ..modules.builder import build_seg_decoder
...
class BevFeatureSlicer(nn.Module):
    ...
    def __init__(self, det_grid_conf, map_grid_conf):
        super().__init__()

        if det_grid_conf == map_grid_conf:
            ...
            bev_resolution, bev_start_position, bev_dimension = calculate_birds_eye_view_parameters(
                det_grid_conf['xbound'], det_grid_conf['ybound'], det_grid_conf['zbound'],
            )
...
class BEVFormerHead(DETRHead):
    ...
    def __init__(self,
        ...
        if 'type' in bbox_coder:
            self.bbox_coder = build_bbox_coder(bbox_coder) # mmdet3d_plugin.core.bbox.coders.nms_free_coder.py
            # point cloud range
        ...
        if self.task.get('seg'):
            ...
            self.seg_decoder = build_seg_decoder(seg_decoder)
    ...
    def forward(self, mlvl_feats, img_metas, prev_bev=None,  only_bev=False):
        ...
        bev_pos = self.positional_encoding(bev_mask).to(dtype) # see mmdet.models.utils.positional_encoding

        if only_bev:  # only use encoder to obtain BEV features
            # see mmdet3d_plugin.bevformer.modules.transformer.py
        ...
        if self.task.get('seg'):
            ...
            outs.update({'seg_preds': seg_pred})
    ...
    def seg_loss(self, semantic_gt, pred_dicts):
        ...
        # assert seg_preds.size() == semantic_gt.size()
