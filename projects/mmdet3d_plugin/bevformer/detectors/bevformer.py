...
class BEVFormer(MVXTwoStageDetector):
    ...
    def extract_img_feat(self, img, img_metas, len_queue=None):
        ...
        B = img.size(0) # (bs*len_queue, num_cams, C, H, W)
        ...
        if img is not None:
            if img.dim() == 5 and img.size(0) == 1:
                ...
                B, N, C, H, W = img.size()
                img = img.reshape(B * N, C, H, W) # (bs*len_queue*num_cams, C, H, W)
    ...
    def forward_pts_train(self,
        ...
        if self.gd_size is not None:
            semantic_indices = semantic_indices[:,:, self.gd_size[0]:self.gd_size[1]]
    ...
    def obtain_history_bev(self, imgs_queue, img_metas_list):
        ...
        with torch.no_grad():
            ...
            # List(multi-level featurs (bs, queue, num_cams, C, H, W))
            img_feats_list = self.extract_feat(img=imgs_queue, len_queue=len_queue) 

            for i in range(len_queue): # fuse history BEV features iteratively
                img_metas = [each[i] for each in img_metas_list]
                if not img_metas[0]['prev_bev_exists']:
                    prev_bev = None
                img_feats = [each_scale[:, i] for each_scale in img_feats_list]

                # only_bev=True -> only run encoder part
                # prev_bev=None -> TSA -> SA
    ...
    def forward_train(self,
        ...
        Args:
            ...
            semantic_indices (list[torch.Tensor], optional): Ground truth 
                for semantic masks. 
        ...
        # 1. Image queue = history frame + current frame
        # img shape = (B, queue, num_cams, C, H, W)
        len_queue = img.size(1)
        prev_img = img[:, :-1, ...] # history
        img = img[:, -1, ...]       # current

        # 2. Obtain the history BEV features
        ...
        # 3. Img backbone to extract 2d img features for current frame
        img_feats = self.extract_feat(img=img, img_metas=img_metas)

        # 4. forward pass
        # 2D img featurs + history BEV -> Encoder (BEVFormerHead, transformer-based) 
        #                                -> Decoder (detection + segmentation heads)
        losses = dict()
