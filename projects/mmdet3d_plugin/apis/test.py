...
def show_seg(labels, car_img, car_view):
    ...
    if car_view in ['full']:
        ...
        img[img.shape[0] // 2 - 8: img.shape[0] // 2 + 8, 0:15, :] = car_img[:,15:,:]
    return img
...
def single_gpu_test(model,
    ...
    for i, data in enumerate(data_loader):
        ...
        if (result[0]['seg_preds'] is not None) and (show or out_dir):
            car_img_cv = cv2.imread(os.path.join(os.getcwd(), 'projects/mmdet3d_plugin/apis/icon/car.png'))
            ...
            if cv2_style:
                ...
                cv2.imwrite(imname, show_seg(semantic.squeeze(), car_img_cv, car_view))

                if show_mask_gt:
                    if model.module.gd_size is not None:
                        target_semantic_indices = data['semantic_indices'][0][:,:,model.module.gd_size[0]:model.module.gd_size[1]].unsqueeze(0)
                    else:
        ...
        if map_enable:
            ...
            if model.module.gd_size is not None:
                target_semantic_indices = data['semantic_indices'][0][:,:,model.module.gd_size[0]:model.module.gd_size[1]].cuda()
            else:
